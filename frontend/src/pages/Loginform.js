...
const LoginForm = ({ onLogin }) => {
  ...
  const validateUsernameOrEmail = (value) => {
    ...
    if (value.length < 3) {
      setUsernameOrEmailError("Bitte ein @ enhalten und mindestens 4 Zeichen eingeben");
        return false;
        alert("Bitte ein gültiges E-Mail Adresse eingeben");
    }
  ...
  const validatePassword = (value) => {
    ...
    if (value.length < 6) {
        ...
        alert("Das Password ist zu kurz bitte gültiges eingeben");
      return false;
    }
    setPasswordError("");
    return true;
